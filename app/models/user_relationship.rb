class UserRelationship < ActiveRecord::Base
  ...
  def self.request(user1, user2)
    UserRelationship.transaction do
      UserRelationship.create(user_id: user2.id, follower_id: user1.id, state: 'following')
      UserRelationship.create(user_id: user1.id, follower_id: user2.id, state: 'followed')
